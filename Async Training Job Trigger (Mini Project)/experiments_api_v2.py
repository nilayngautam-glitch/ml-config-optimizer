...
from celery_worker import run_train_job
...
@app.post("/experiments/submit", status_code=202)
async def submit(*,session: Session = Depends(get_session), exp_insert : Experiment_insert ):
    config_dict = exp_insert.model_dump()
    task = run_train_job.delay(config_dict)
